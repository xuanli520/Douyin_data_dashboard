def test_register_jobs_contains_dashboard_full_incremental_cookie_agent(monkeypatch):
    ...
    assert "shop_dashboard_full_sync" in job_ids
    assert "shop_dashboard_incremental_sync" in job_ids
    assert "shop_dashboard_cookie_health_check" in job_ids
    assert "shop_dashboard_agent_backfill" in job_ids
