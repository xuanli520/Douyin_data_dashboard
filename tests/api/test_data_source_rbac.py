...
def test_list_data_sources_requires_permission(rbac_client):
    response = rbac_client.get("/api/v1/data-sources")
    assert response.status_code == 401


def test_create_data_source_requires_permission(rbac_client):
    response = rbac_client.post("/api/v1/data-sources", json={"name": "test"})
    assert response.status_code == 401


def test_get_data_source_requires_permission(rbac_client):
    response = rbac_client.get("/api/v1/data-sources/1")
    assert response.status_code == 401


def test_update_data_source_requires_permission(rbac_client):
    response = rbac_client.put("/api/v1/data-sources/1", json={"name": "updated"})
    assert response.status_code == 401
